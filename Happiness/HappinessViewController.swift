...
class HappinessViewController: UIViewController, FaceViewDataSource
{
    ...
    //when this happiness is first set the property observer says to updateUI().  Well, we got here from preparation.  We are preparing for the segue and we tell the model to prepare and we set the happiness and it is supposed to prep the segue.  Well, the view is nothing yet and there is no faceView to draw.  When you are preparing, the outlet has for the faceView has not been set, so faceView is nil
        //there is a property observer here.  Anytime someone sets happy, we want the tittle of the HappinessViewConroller to update its title  The model is here.  It is not another object.  our model just resides in the controller.  It is simply happiness.  And dont get this model confused with smiliness.  That is just the arc in the FaceView.  So, the controller has to interpret the happiness into the smiliness.  (there are other things to happiness such as crinkly eyes.  So, smile is somehthign differet.
    var happiness: Int = 22
        {                           //0 = very sad, 100 = happ
        //use a property observer here to monitor / keep the integrity of the model.  
        didSet{
    ...
    @IBOutlet weak var faceView: FaceView! {
        ...
        didSet{
            ...
            print("in HapVwContrler & in prop. observer of @outlet var faceView and adfa is \(adfa)")
